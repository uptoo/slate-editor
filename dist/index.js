...
export default function MyEditor({
  ...
  // Editeur
  const editor = useMemo(() => withMentions(withReact(withHistory(createEditor()))), []); // Valeur du contenu

  const [value, setValue] = useState(content); // Pour les mentions

  const ref = useRef();
  const [target, setTarget] = useState();
  const [index, setIndex] = useState(0);
  const [search, setSearch] = useState(''); // Fonctions de render
  ...
  const users = mentions.filter(m => m.firstName.toLowerCase().startsWith(search.toLowerCase())).slice(0, 10);
  ...
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement(MarkButton, {
    format: "bold"
  ...
  }), target && users.length > 0 && /*#__PURE__*/React.createElement("div", {
    ref: ref,
    style: {
      top: '-9999px',
      left: '-9999px',
  ...
  }, users.map((user, i) => /*#__PURE__*/React.createElement("div", {
    key: user._id,
    style: {
      padding: '1px 3px',
      borderRadius: '3px',
...
const Element = props => {
  ...
  switch (element.type) {
    case 'mention':
      return /*#__PURE__*/React.createElement(Mention, props);
